...
//            Setup.ClientsCS = _clients;
            ...
            Client client)
        {
            ...
            extendedPacketInformation.Ip = client.Ip;
            extendedPacketInformation.Port = client.Port;
            extendedPacketInformation.UdpMessageType = UdpMessageType.Packet;
            extendedPacketInformation.Packet.SeqNo = 1;
            extendedPacketInformation.Packet.ReturnPort = client.Port;
            A.CallTo(() => harakaDb.GetObjects<Client>(A<string>.Ignored)).Returns(new List<Client>() {client});
            A.CallTo(() => guranteedDelivery.RemoveMessageFromReceiveQueueAsync(extendedPacketInformation.Id)).Returns(new Task(() => {}));

            sut.HandleRecievedMessage(extendedPacketInformation);
